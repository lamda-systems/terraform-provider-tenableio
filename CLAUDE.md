...
- An aliased provider that declares no proxy settings of its own inherits the shared `TENABLEIO_PROXY_AUTH_*` pair — the fallback is not special-cased for credentials. To opt a single instance *out* of an inherited proxy, set both attributes to `""` in its provider block: a configured attribute beats the environment even when empty. Setting only one to `""` is rejected, because the other half would still be inherited.
- **Sharing happens through the environment only.** Terraform configures each provider block independently, so an attribute written into the default `provider "tenableio"` block is invisible to every aliased block — the provider never sees it. Splitting the pair across sources is the trap this causes: header in the default block's HCL, value in `TENABLEIO_PROXY_AUTH_VALUE`, and every alias resolves a value with no header and fails with "Missing Proxy Auth Header". Both proxy diagnostics point this out via `aliasHint`. Anything meant to be shared across instances belongs in an environment variable, not in one provider block.
